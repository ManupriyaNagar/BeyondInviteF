...
const handleLike = async (templateId) => {
  ...
  if (!currentUser) {
    ...
    const res = await fetch(`http://localhost:5001/api/auth/check?email=${email}`);
...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`http://localhost:5001/api/templates/${params.id}`);
        ...
        const resAll = await fetch("http://localhost:5001/api/templates");
  ...
  className={`flex-1 border py-3 rounded-lg flex items-center justify-center gap-2 transition-colors ${
    isLiked ? "border-red-500 text-red-500 bg-red-50" : "border-gray-300 text-gray-700 hover:bg-gray-50"
  }`}
>
                ...
                .map(rel => (
                  ...
                  >
                    <div className="aspect-square relative">
                      {rel.image_url && (
                        <Image
                          src={rel.image_url.startsWith("http") ? rel.image_url : `http://localhost:5001${rel.image_url}`}
                          alt={rel.title}
                          fill
